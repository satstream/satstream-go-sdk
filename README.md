...
```go
params := FooParams{
	Name: satstream.F("hello"),

	// Explicitly send `"description": null`
	Description: satstream.Null[string](),

	Point: satstream.F(satstream.Point{
		X: satstream.Int(0),
		Y: satstream.Int(1),

		// In cases where the API specifies a given type,
		// but you want to send something else, use `Raw`:
		Z: satstream.Raw[int64](0.01), // sends a float
	}),
}
```
...
`*satstream.Error`. This contains the `StatusCode`, `*http.Request`, and
...
We also provide a helper `satstream.FileParam(reader io.Reader, filename string, contentType string)`
...
```go
params := FooNewParams{
    ID:   satstream.F("id_xxxx"),
    Data: satstream.F(FooNewParamsData{
