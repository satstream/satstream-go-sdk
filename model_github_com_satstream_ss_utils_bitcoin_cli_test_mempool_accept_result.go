/*
 * Satstream API
 *
 * Satstream API
 *
 * API version: 1.0
 * Contact: team@satstream.io
 * Generated by: Swagger Codegen (https://github.com/swagger-api/swagger-codegen.git)
 */
package satstream_go_sdk

type GithubComSatstreamSsUtilsBitcoinCliTestMempoolAcceptResult struct {
	// Whether this tx would be accepted to the mempool
	Allowed bool `json:"allowed,omitempty"`
	// Transaction fees
	Fees *AllOfgithubComSatstreamSsUtilsBitcoinCliTestMempoolAcceptResultFees `json:"fees,omitempty"`
	// Package validation error, if any
	PackageError string `json:"package-error,omitempty"`
	// Rejection string
	RejectReason string `json:"reject-reason,omitempty"`
	// The transaction hash in hex
	Txid string `json:"txid,omitempty"`
	// Virtual transaction size as defined in BIP 141
	Vsize int32 `json:"vsize,omitempty"`
	// The transaction witness hash in hex
	Wtxid string `json:"wtxid,omitempty"`
}
